setwd("C:/Users/amykr/Desktop/rworkshop/feb13")
---
...
install.packages(c("sp", "sf", "rgdal", "classInt", "RColorBrewer", "ggplot", "ggmap", "leaflet", "broom", "tmap", "GISTools"))
library(sp)
library(sf)
library(rgdal)
library(classInt)
...
denv_c<-read.csv("C:/Users/amykr/Google Drive/Kent/james/dissertation/chkv and dengue/arcgis analysis/gwr models/output/gwr4nov16/dengue/dengue_c_collapsed.csv")%>%filter(codigo_barrio!=2300)
denv_c$denv_c_inc<-denv_c$dengue_c_sum/denv_c$total_pop
denv_c$denv_c_inc_100000<-(denv_c$dengue_c_sum/denv_c$total_pop)*100000
table(denv_c$denv_c_inc_100000)
chikvincidence <- read.csv("C:/Users/amykr/OneDrive/epi analysis/diseasebarrio/chikv_barriofeb202017.csv", header=TRUE)

denvincidence <- read.csv("C:/Users/amykr/OneDrive/epi analysis/diseasebarrio/dengue_barriofeb202017.csv", header=TRUE)

zikaincidence <- read.csv("C:/Users/amykr/OneDrive/epi analysis/diseasebarrio/zika_barriofeb202017.csv", header=TRUE)


library(stringr)
zikaincidence$ID_barrio4<-str_pad(zikaincidence$ID_barrio4, 4, pad = "0")
denv_c$ID_barrio4<-str_pad(denv_c$codigo_barrio, 4, pad = "0")
barrios <- readOGR("C:/Users/amykr/Google Drive/Kent/james/dissertation/chkv and dengue/arcgis analysis/data/avg_rain.gdb", "barrioswgs1984", verbose = F)

chikvincidence <- merge(barrios,chikvincidence, by.x='ID_BARRIO', by.y='ID_barrio4')
denvincidence <- merge(barrios,denvincidence, by.x='ID_BARRIO', by.y='ID_barrio4')
denv_c_incidence <- merge(barrios,denv_c, by.x='ID_BARRIO', by.y='ID_barrio4')
zikaincidence <- merge(barrios,zikaincidence, by.x='ID_BARRIO', by.y='ID_barrio4')


install.packages(c("rgdal", "sp"))
library(rgdal)
library(sp)
writeOGR(zikaincidence, "C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles", "zika", driver = "ESRI Shapefile")
save(zikaincidence, file="C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles/zikaincidence.R")        # save to the working directory

writeOGR(denvincidence, "C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles", "denv", driver = "ESRI Shapefile")
save(denvincidence, file="C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles/denvincidence.R")        # save to the working directory

writeOGR(denv_c_incidence, "C:/Users/amykr/Box Sync/Amy Krystosik's Files/cali- epi of arboviruses/epi paper/figures/Figure 3 editable shapefiles and spatial objects and code", "denv_c_incidence", driver = "ESRI Shapefile")
save(denv_c_incidence, file="C:/Users/amykr/Box Sync/Amy Krystosik's Files/cali- epi of arboviruses/epi paper/figures/Figure 3 editable shapefiles and spatial objects and code/denv_c_incidence.R")        # save to the working directory

writeOGR(chikvincidence, "C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles", "chikv", driver = "ESRI Shapefile")
save(chikvincidence, file="C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles/chikvincidence.R")        # save to the working directory

load(file="C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles/denvincidence.R")
load(file="C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles/chikvincidence.R")
load(file="C:/Users/amykr/OneDrive/epi paper/figures/editable map shapefiles/zikaincidence.R")


```
crudeincidence_barriosum1000000# 1. Choropleth mapping with `spplot` 
...
spplot(chikvincidence, "crudeincidence_barriosum1000", main = "Chikunguya (incidence per 1,000)")
spplot(denvincidence, "crudeincidence_barriosum1000", main = "Dengue (incidence per 1,000)")
spplot(zikaincidence, "crudeincidence_barriosum1000", main = "Zika (incidence per 1,000)")
...
library(RColorBrewer)
display.brewer.all(type="seq")
pal <- brewer.pal(9, "YlOrRd") # we select 5 colors from the palette
class(pal)
```
...
```{r eval=FALSE}
spplot(denvincidence,"crudeincidence_barriosum1000", col.regions = pal, cuts = 8, main = "Dengue incidence per 1,000 population", sub = "(Nov '15 -Apr '16), Source: SIVIGILA, DANE") 
spplot(chikvincidence,"crudeincidence_barriosum1000", col.regions = pal, cuts = 8, main = "Chikungunya incidence per 1,000 population", sub = "(Jan '15 -Apr '16)", Source: SIVIGILA, DANE) 
spplot(zikaincidence,"crudeincidence_barriosum1000", col.regions = pal, cuts = 8, main = "Zika incidence per 1,000 population", sub = "(Nov '15 -Apr '16)", Source: SIVIGILA, DANE)
...
```{r eval=FALSE}
spplot(denvincidence,"crudeincidence_barriosum1000", cuts = 8, col.regions = pal, at = denv_breaks$brks, main = "Dengue incidence per 1,000", sub = "(Nov '15 -Apr '16)") 
spplot(chikvincidence,"crudeincidence_barriosum1000", cuts = 8, col.regions = pal, chikv_breaks$brks, main = "Chikungunya incidence per 1,000", sub = "(Jan '15 -Apr '16)") 
spplot(zikaincidence,"crudeincidence_barriosum1000", cuts = 8, col.regions = pal, zika_breaks$brks, main = "Zika incidence per 1,000", sub = "(Nov '15 -Apr '16)")
...
spplot(denvincidence,"crudeincidence_barriosum1000", cuts = 8, col.regions = pal, at = denvbr, main = "Dengue incidence per 1,000", sub = "(Nov '15 -Apr '16), Source: SIVIGILA & DANE") 
spplot(chikvincidence,"crudeincidence_barriosum1000", cuts = 8, col.regions = pal, at = chikvbr, main = "Chikungunya incidence per 1,000", sub = "(Jan '15 -Apr '16), Source: SIVIGILA & DANE") 
spplot(zikaincidence,"crudeincidence_barriosum1000", cuts = 8, col.regions = pal, at = zikabr, main = "Zika incidence per 1,000", sub = "(Nov '15 -Apr '16), Source: SIVIGILA & DANE")
```
...
tiff("C:/Users/amykr/OneDrive/epi paper/figures/denvincidence.tiff", height = 12, width = 17, units = 'cm', compression = "lzw", res = 600)
...
tiff("C:/Users/amykr/OneDrive/epi paper/figures/chikvincidence.tiff", height = 12, width = 17, units = 'cm', compression = "lzw", res = 600)
...
tiff("C:/Users/amykr/OneDrive/epi paper/figures/zikaincidence.tiff", height = 12, width = 17, units = 'cm', compression = "lzw", res = 600)
...
```{r echo=FALSE}
philly <- readOGR("C:/Users/amykr/Desktop/rworkshop/feb13/Philly3/", "Philly3", verbose = F)
...
library(RColorBrewer)
library(classInt)
philly <- readOGR("C:/Users/amykr/Desktop/rworkshop/feb13/Philly3", "Philly3") #
pal <- brewer.pal(5, "OrRd")
breaks_qt <- classIntervals(philly$HOMIC_R, n = 5, style = "quantile")
br <- breaks_qt$brks 
offs <- 0.0000001 
...
denvincidence$crudeincidence_barriosum1000 [denvincidence$crudeincidence_barriosum1000=NaN] <- 0
choropleth(denvincidence, denvincidence$crudeincidence_barriosum1000, na.rm=TRUE)              # plot the polygons
shd <-  auto.shading(denvincidence$crudeincidence_barriosum1000)            # we need that for the legend coloring
...
st_read
library(sf)
philly_sf <-  st_read("C:/Users/amykr/OneDrive/epi paper/figures/zika/layer name.shp")
plot(philly_sf)
```
...
philly_sf <-  st_read("C:/Users/amykr/Desktop/rworkshop/feb13/Philly3/Philly3.shp", quiet = T)
hr_cuts <-  cut(philly_sf$HOMIC_R, br)
plot(philly_sf["HOMIC_R"], main = "Philadelphia homicide rate per 100,000", col = pal[as.numeric(hr_cuts)])
legend(1760000, 471000, legend = paste("<", round(br[-1])), fill = pal)                       
```
...
library(tmap)
tm_shape(philly) +
  tm_polygons("HOMIC_R", style="quantile", title="Philadelphia /nhomicide rate /nper 100,000")
```
...
6. Ok, so this is a bit annoying, since the labels of the legend show percentages instead of the actual value breaks. 
    ...
    "By default, labFormat is basically format(scientific = FALSE,big.mark = ',') for the numeric palette, as.character() for the factor palette, and a function to return labels of the form âx[i] - x[i + 1]â for bin and quantile palettes (__in the case of quantile palettes, x is the probabilities instead of the values of breaks__)"
