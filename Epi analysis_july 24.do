...
****************************************************************incidence****************************************************************
		*pop standardization incidence by agecatepi and sex***
			**import populations by age sex and year**
	...
	***open case data***
		use "dengue_chikv_oct2014-oct2015_cali.dta", replace
			tostring Age_Categories, replace
			destring anos, replace
			tostring edad_cat_epi, replace

****merge the population data to the case data*********
	...
	*crude incidence table by age
		...
		export excel using "cases_incidence.xls", firstrow(variables) replace 
		...
		export excel using "incidence.xls", firstrow(variables) replace 
...
*create collapsed outcome category of chikv and dengue without and with severity and grave and death. 
	...
	*outcome = 1 for dengue without warning
	replace outcome_collapsed_2 =1 if clasfinal == 1
	*outcome = 2 for dengue with warning signs
	replace outcome_collapsed_2 =2 if clasfinal == 2
	*outcome = 3 for dengue grave
	replace outcome_collapsed_2 = 3 if dengue_death3 == 1
	*outcome = 3 for dengue death
	replace outcome_collapsed_2 = 3 if dengue_death4 == 1
	*outcome = 5 for dengue unclassified
	replace outcome_collapsed_2  =5 if cod_eve == 210 & outcome ==.
	*outcome = 6 for chikungunya 
	replace outcome_collapsed_2 = 6 if  cod_eve == 217
	*outcome = 7  for zika
	replace outcome_collapsed_2 = 7 if cod_eve == 895
...
*table incidence table with f to m ratios for nom_eve
	...
	export excel using "weightedincidenceratioscollapsed.xls", firstrow(variables) replace 
...
*table incidence table with f to m ratios for outcome_collapsed
	use temp.dta, clear
		...
		gen upper = .
		replace upper = ratioftom + 1.96*sqrt((1/weightedincidencemale)  + (1/weightedincidencefemale))
	export excel using "weightedincidenceratiosoutcome_collapsed.xls", firstrow(variables) replace
